...
public class LinkedList {
	...
	public int getMinIter() throws ListEmptyException {
		...
		int minKey = anchor.getKey();
		
		ListNodeSL ele = anchor;
		
		while((ele = ele.getNext()) != null)
			minKey = Math.min(minKey, ele.getKey());
		
		return minKey;
	}
	
	public int getMinRec(ListNodeSL sl) throws ListEmptyException {
		if (isEmpty()) throw new ListEmptyException(); 
		
		if (sl.getNext() == null) 
			return sl.getKey(); 
		return Math.min(sl.getKey(), getMinRec(sl.getNext()));
	}
	
	public void printReversed(ListNodeSL sl) {
		if (sl.getNext() != null) 
			printReversed(sl.getNext());
	...
	public void reverse() {
		...
		ListNodeSL last = anchor;
		ListNodeSL next = last.getNext();
		anchor.setNext(null);
		
		while(next != null) {
			ListNodeSL tmp = next.getNext(); 
